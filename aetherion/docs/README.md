...
افتح `simulator/index.html` عبر الخادم المحلي (لا تفتحه كـ `file://` حتى يعمل استيراد JSON).

- شارت متوهج بصفقات حية  
- سبريد حي يتغيّر مع كل شمعة  
- استيراد تقارير المختبر الحقيقية (أسقط `Tester/Report_*.htm`)  
- تصدير CSV و PNG  
- مقارنة كل الرموز × كل الأطر على نفس الرسم
